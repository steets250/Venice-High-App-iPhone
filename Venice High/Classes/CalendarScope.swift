...
class CalendarScope: UIViewController, UIGestureRecognizerDelegate {
    ...
    init() {
        ...
        selectButton.titleLabel!.font = UIFont(name: selectButton.titleLabel!.font.fontName, size: 25.0)
        ...
        headerLabel.theme_textColor = ThemeColorPicker(keyPath: "Global.textColor")
        bodyLabel.theme_textColor = ThemeColorPicker(keyPath: "Global.textColor")

        if defaults.bool(forKey: "Is Dark") {
            contentView.backgroundColor = .black
            pickerView.selectorColor = .white
            cancelButton.theme_setTitleColor(ThemeColorPicker(keyPath: "Global.themeBlue"), forState: .normal)
            selectButton.theme_setTitleColor(ThemeColorPicker(keyPath: "Global.themeBlue"), forState: .normal)
        } else {
    ...
    @objc func complete() {
        ...
        if row == 0 && nameSet == false {
            let alert = PMAlertController(title: "New Calendar", message: "Enter a name for your calendar.", preferredStyle: .alert, preferredTheme: appDelegate.themeAlert)
            alert.dismissWithBackgroudTouch = false
            alert.addTextField {textField in
                textField?.placeholder = "Venice High School"
            }
            alert.addAction(PMAlertAction(title: "OK", style: .default, handler: {
            ...
            do {
                ...
                let alert = PMAlertController(title: "Calendar Error", message: (error as NSError).description.slice(from: "\"", to: "\"")!, preferredStyle: .alert, preferredTheme: appDelegate.themeAlert)
                let OKAction = PMAlertAction(title: "OK", style: .default, handler: nil)
    ...
    @objc func hide() {
        if defaults.string(forKey: "Calendar Name") != nil {
...
extension CalendarScope: UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, attributedTitleForRow row: Int, forComponent component: Int) -> NSAttributedString? {
        ...
        let myTitle = NSAttributedString(string: titleData, attributes: [NSAttributedStringKey.foregroundColor: color])
        return myTitle
    }
}
